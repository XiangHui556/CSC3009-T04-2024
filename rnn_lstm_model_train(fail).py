...
data_dir_train = '/content/drive/MyDrive/Colab Notebooks/MRI_Images/resized_dataset_128/train'
data_dir_val = '/content/drive/MyDrive/Colab Notebooks/MRI_Images/resized_dataset_128/val'
...
class LSTM(nn.Module):
    ...
    def forward(self, X):
        hidden_states = torch.zeros(self.n_layers, X.size(0), self.hidden_size).to(device)
...
for params in ParameterGrid(param_grid):
    ...
    for epoch in range(max_epoch):
        ...
        for batch, (image, label) in enumerate(train_loader):
            image = image.reshape(-1, 128, 128).to(device)  # reshape the image so that it can fit into the LSTM model
        ...
        print(f"Epoch {epoch + 1}, LR: {learning_rate}, hidden_layers: {hidden_layers}, n_layers: {n_layers}, Validation Accuracy: {val_accuracy * 100:.2f}%, Validation Loss: {avg_val_loss:.6f}")
